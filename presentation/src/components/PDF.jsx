...
const PDF = (criticalCount, highCount, mediumCount, lowCount, trendAttacks, networkStatus, srcAndDstIP, answerRecommendation, answerConclusion, answerTrend, openCount, inProgressCount, resolvedCount, falsePositiveCount, severityLevel, predictedAttack, formattedConfidence) => {
  const doc = new jsPDF({
  ...
  const justifyText = (doc, text, startX, y, maxWidth) => {
    ...
    for (let word of words) {
      ...
      if (testWidth > maxWidth && line) {
        ...
        const lineWidth = lineWords.reduce((acc, word) => acc + doc.getStringUnitWidth(word) * doc.internal.getFontSize() / doc.internal.scaleFactor, 0);
  ...
  currentHeight += 5;
  doc.setFont('light').setFontSize(10).text(`Reporting Period: ${currentTime}`, startX, currentHeight);
  currentHeight += 10;
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11);
  const summaryHeight = justifyText(doc, 
  ...
  const tableBody = srcAndDstIP.map((srcIP) => {
    return [
      srcIP.src_addr,
      srcIP.count_src_addr,
      srcIP.dst_addr,
      srcIP.count_dst_addr
    ];
  });
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11).text(
    `The EyeSeeYou system detected a total of ${criticalCount+highCount+mediumCount+lowCount} alerts categorized by severity as follows:`,
    startX,
    currentHeight
  );
  currentHeight += 5;
  ...
  currentHeight += 5;
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11).text("The following are the current statuses of the alerts:", startX, currentHeight);
  currentHeight += 5;
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11);
  const mlAnalysisHeight = justifyText(doc,
  ...
  currentHeight += 5;
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11).text("The following trends were observed in detected threats:", startX, currentHeight);
  currentHeight += 5;
  ...
  currentHeight += 5;
  const trendTextHeight = justifyText(doc,
    answerTrend,
    25,
    currentHeight,
    endX - startX - 5
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11);
  const recommendationTextHeight = justifyText(doc,
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11);
  const conclusionTextHeight = justifyText(doc,
    answerConclusion,
    25,
    currentHeight,
    endX - startX - 5
