...
const PDF = (criticalCount, highCount, mediumCount, lowCount, trendAttacks, networkStatus, srcAndDstIP, answerRecommendation, answerConclusion, answerTrend) => {
  const doc = new jsPDF({
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11).text(`The EyeSeeYou system detected a total of ${criticalCount+highCount+mediumCount+lowCount} alerts categorized by severity as follows:`, startX, 160);
  ...
  doc.text(`• Open:`, 25, 200);
  doc.text(`• In Progress:`, 25, 205);
  doc.text(`• Resolved:`, 25, 210);
  doc.text(`• False Positive:`, 25, 215);
  ...
  doc.setFont("Times-Roman", "normal");
  doc.setFontSize(11);
  const recommendationTextHeight = doc.splitTextToSize(answerRecommendation, endX - startX - 5).length * lineHeight; // Calculate height needed for recommendation text
  justifyText(doc, 
