...
function GeneratePDF() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const sortedTrendAttacks = response.data.list_trending_attacks.sort(
            (a, b) => b.count - a.count
          );
  ...
  useEffect(() => {
    const generateAIContent = async () => {
      ...
      const topSrcIpList = srcAndDstIP
        .map((ip) => `${ip.src_addr} with the count ${ip.count_src_addr}`)
        .join(", ");
      const topDstIpList = srcAndDstIP
        .map((ip) => `${ip.dst_addr} with the count ${ip.count_dst_addr}`)
        .join(", ");
      const trendAttacksList = trendAttacks
        .map((ip) => `${ip.class} with the count ${ip.count}`)
        .join(", ");
      ...
      const context = `
            ...
            EyeSeeYou system detected a total of ${
              alertsOverview.critical +
              alertsOverview.high +
              alertsOverview.med +
              alertsOverview.low
            } alerts categorized by severity as follows: Critical: ${
        alertsOverview.critical
      }, High:${alertsOverview.high} , Medium:${alertsOverview.med}, Low:${alertsOverview.low}. 
  ...
  const handlePrint = () => {
    PDF(
      ...
      answerTrend,
      alertsStatus.open,
      alertsStatus.inProgress,
      alertsStatus.resolved,
      alertsStatus.falsePositive,
      severityLevel,
      predictedAttack,
      formattedConfidence
    );
      ...
      <div className="flex flex-col min-h-screen bg-[#f4f4f4] dark:bg-[#1C1D1F] text-black dark:text-white px-4 sm:px-6 md:px-8 lg:px-12">
        ...
        <div className="flex mt-4 mb-4">
          ...
          <button
            ...
            disabled={isLoading}>
            <svg
              className="w-6 h-6 text-gray-800 hover:text-blue-700 dark:text-white dark:hover:text-blue-700 justify-start transition duration-300 "
              aria-hidden="true"
              ...
              <div className="grid grid-cols-1 gap-8 lg:w-2/3 text-black">
                ...
                <div className="flex flex-col h-[1100px] bg-white shadow-lg rounded-lg">
                  <div className="flex-grow overflow-hidden px-14 py-10">
                    ...
                    <h1 className="text-[20px] font-bold text-center">
                      EyeSeeYou Network Analysis Report
                    </h1>
                    ...
                    <p className="text-[11px] text-gray-800 text-justify">
                      This report provides a comprehensive overview of network activity monitored by
                      the EyeSeeYou intrusion detection system. It highlights recent security
                      alerts, trends in network activity, and recommendations for enhancing overall
                      security posture. Over the reporting period, the system detected a total of{" "}
                      {alertsOverview.critical +
                        alertsOverview.high +
                        alertsOverview.med +
                        alertsOverview.low}{" "}
                    ...
                    <p className="text-[11px] text-gray-800">
                      During the reporting period, the EyeSeeYou dashboard tracked a variety of
                    ...
                    <p className="text-[11px] text-gray-800">
                      The following are the current statuses of the alerts:
                    </p>
                    <ul className="list-disc list-inside ml-4">
                      <li className="text-[11px]">
                    ...
                    {/* ML analysis */}
                    <h3 className="text-[13px] font-semibold mt-4">
                      2.3 Analysis of Historical Patterns
                    </h3>
                    <p className="text-[11px] text-gray-800">
                      Following up on the recent analysis, the system has detected an{" "}
                      {predictedAttack} pattern with a HIGH severity level. The confidence level of
                      the detection stands at {formattedConfidence}%, indicating a significant risk.
                    </p>
                    <ul className="list-disc list-inside ml-4">
                      <li className="text-[11px]">
                  ...
                  <footer className="text-[9px] px-14 py-10">
                    {" "}
                    © 2024 EyeSeeYou. All rights reserved.
                  </footer>
                </div>
                ...
                <div className="h-[1100px] bg-white shadow-lg rounded-lg flex flex-col mb-8">
                  <div className="flex-grow overflow-hidden px-14 py-10">
                    ...
                    <p className="text-[11px] text-gray-800">
                      The following trends were observed in detected threats:
                    </p>
                    <ul className="list-disc list-inside ml-4">
